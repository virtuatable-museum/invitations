RSpec.shared_examples 'from request to accepted' do
  describe 'Update from request to accepted' do
    describe 'update by the user' do
      before do
        put "/#{request_invitation.id.to_s}", {session_id: account_session.token, app_key: 'test_key', token: 'test_token', status: 'accepted'}
      end
    ...
    describe 'update by the creator' do
      before do
        put "/#{request_invitation.id.to_s}", {session_id: creator_session.token, app_key: 'test_key', token: 'test_token', status: 'accepted'}
      end
