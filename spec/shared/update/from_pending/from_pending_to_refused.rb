RSpec.shared_examples 'from pending to refused' do
  describe 'Update from pending to refused' do
    describe 'update by the user' do
      before do
        put "/invitations/#{pending_invitation.id.to_s}", {session_id: account_session.token, app_key: 'test_key', token: 'test_token', status: 'refused'}
      end
    ...
    describe 'update by the creator' do
      before do
        put "/invitations/#{pending_invitation.id.to_s}", {session_id: creator_session.token, app_key: 'test_key', token: 'test_token', status: 'refused'}
      end
