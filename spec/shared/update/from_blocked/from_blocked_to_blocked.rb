RSpec.shared_examples 'from blocked to blocked' do
  describe 'Update from blocked to blocked' do
    describe 'update by the user' do
      before do
        put "/invitations/#{blocked_invitation.id.to_s}", {session_id: account_session.token, app_key: 'test_key', token: 'test_token', status: 'blocked'}
      end
    ...
    describe 'update by the creator' do
      before do
        put "/invitations/#{blocked_invitation.id.to_s}", {session_id: creator_session.token, app_key: 'test_key', token: 'test_token', status: 'blocked'}
      end
      it 'Returns a OK (200) status' do
        expect(last_response.status).to be 200
      end
